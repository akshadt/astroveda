...
export default function GemstonesManagement() {
  const router = useRouter();
  const [gemstonesList, setGemstonesList] = useState<Product[]>([]);
  ...
  const openEditModal = (gemstone: Product) => {
    setEditingId(gemstone._id || gemstone.id || null);
    setFormData({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold font-playfair text-[#0F172A]">Gemstones Management</h1>
          <p className="text-[#64748B] text-sm mt-1">Manage astrology gemstones inventory and details.</p>
        </div>
        <button
        ...
        {loading ? (
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
              <tr>
                <th className="px-6 py-4 font-bold text-[#64748B]">Gemstone</th>
            ...
            <tbody className="divide-y divide-[#E2E8F0]">
              {gemstonesList.length > 0 ? (
                gemstonesList.map((gem) => (
                  <tr key={gem._id || gem.id} className="hover:bg-[#F8FAFC]">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <img src={gem.image || "https://picsum.photos/seed/placeholder/100/100"} alt="" className="w-10 h-10 rounded-md object-cover mr-3 border border-[#E2E8F0]" />
                        <div className="font-bold text-[#0F172A] truncate max-w-[200px]">{gem.title}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[#64748B]">{gem.zodiac || "-"}</td>
                    <td className="px-6 py-4 text-[#64748B] truncate max-w-[150px]">{gem.certification || "-"}</td>
                    <td className="px-6 py-4 font-bold text-[#F97316]">₹{gem.price}</td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => openEditModal(gem)} className="text-blue-600 hover:text-blue-800 font-bold mr-4">Edit</button>
                      <button onClick={() => handleDelete(gem._id || gem.id || "")} className="text-red-500 hover:text-red-700 font-bold">Delete</button>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-[#64748B] font-medium">
                    No gemstones found. Add one to get started.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        ...
        title={editingId ? "Edit Gemstone" : "Add New Gemstone"}
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" placeholder="e.g. Leo, Virgo" value={formData.zodiac} onChange={e => setFormData({...formData, zodiac: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none" />
            </div>
            <div>
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. IGI Certified" value={formData.certification} onChange={e => setFormData({...formData, certification: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none" />
          </div>
          <div>
