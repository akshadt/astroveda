  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold font-playfair text-[#0F172A]">Blogs Management</h1>
          <p className="text-[#64748B] text-sm mt-1">Manage astrology blog articles and insights.</p>
        </div>
        <button
        ...
        {loading ? (
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
              <tr>
                <th className="px-6 py-4 font-bold text-[#64748B]">Blog Title</th>
                <th className="px-6 py-4 font-bold text-[#64748B]">Category</th>
                <th className="px-6 py-4 font-bold text-[#64748B]">Date</th>
            ...
            <tbody className="divide-y divide-[#E2E8F0]">
              {blogsList.length > 0 ? (
                blogsList.map((blog) => (
                  <tr key={blog._id || blog.id} className="hover:bg-[#F8FAFC]">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <img src={blog.image || "https://picsum.photos/seed/placeholder/100/100"} alt="" className="w-10 h-10 rounded-md object-cover mr-3 border border-[#E2E8F0]" />
                        <div className="font-bold text-[#0F172A] truncate max-w-[300px]">{blog.title}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[#64748B]">{blog.category}</td>
                    <td className="px-6 py-4 text-[#64748B]">{blog.createdAt ? new Date(blog.createdAt).toLocaleDateString() : "-"}</td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => openEditModal(blog)} className="text-blue-600 hover:text-blue-800 font-bold mr-4">Edit</button>
                      <button onClick={() => handleDelete(blog._id || blog.id || "")} className="text-red-500 hover:text-red-700 font-bold">Delete</button>
                    </td>
                  </tr>
                ))
              ) : (
      ...
      >
        <div className="space-y-4 max-h-[70vh] overflow-y-auto px-1 hide-scrollbar">
          ...
          <div>
            ...
            <input type="text" required value={formData.title} onChange={handleTitleChange} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-[#0F172A] mb-1">Slug</label>
              <input type="text" required value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none" />
            </div>
            <div>
              <label className="block text-sm font-bold text-[#0F172A] mb-1">Category</label>
              <input type="text" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-bold text-[#0F172A] mb-1">Excerpt</label>
            <textarea required value={formData.excerpt} onChange={e => setFormData({...formData, excerpt: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none min-h-[60px]" />
          </div>
          <div>
            <label className="block text-sm font-bold text-[#0F172A] mb-1">Content (HTML or Markdown supported on frontend)</label>
            <textarea required value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} className="w-full px-3 py-2 border border-[#E2E8F0] rounded-lg focus:ring-[#F97316] focus:border-[#F97316] outline-none min-h-[150px] font-mono text-sm" />
          </div>
          <div>
